    ...
    <div className="bg-white rounded-xl p-4 sm:p-6 border border-gray-100 h-full">
      ...
      <div className="space-y-2 sm:space-y-3">
        {LIVE_CONVERSATIONS.map((conv) => (
          <div
            key={conv.id}
            className="flex items-center gap-3 p-2 sm:p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer"
          >
            {/* Avatar */}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-medium text-xs sm:text-sm text-gray-900 truncate">{conv.name}</span>
              </div>
