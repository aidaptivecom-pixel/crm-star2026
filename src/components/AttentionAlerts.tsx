    ...
    <div className="mb-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {ATTENTION_LEADS.map((lead) => (
          <div
            key={lead.id}
            className={`p-4 rounded-xl border ${getReasonBg(lead.reason)} hover:shadow-md transition-all cursor-pointer`}
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  ...
                  {lead.score && (
                    <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${getScoreColor(lead.score)}`}>
                      {lead.score}
                    </span>
                  )}
                </div>
